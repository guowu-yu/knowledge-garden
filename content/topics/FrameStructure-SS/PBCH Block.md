---
title: 5G FrameStructure and SS/PBCH Block
slug: FrameStructure and SSB 
date: 2026-08-12
tags: [5G, FrameStructure, SSB]
summary: 5G 的帧结构构成 以及 SS/PBCH Block 的构成
...
记忆：子载波间隔 = 15 × 2μ kHz；μ 越大，符号越短、时隙越多，越适合高频与低时延。
...
4.3.2 时隙（Slot）
对子载波间隔配置 μ，时隙在子帧内编号为 0 … 2μ−1，在帧内编号为 0 … 10·2μ−1。常规 CP 下每时隙通常为 14 个 OFDM 符号（扩展 CP 为 12）。
...
## 同步信号与小区 ID
物理层小区标识共 1008 个：NIDcell = 3·NID(1) + NID(2)。
PSS：主同步信号，检测 NID(2)（0…2）。
